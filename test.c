...
static FSchar *fgs_file_name = "test";
...
/* *   */
FSvar * _bb_test (parno, parptr)
FSint   parno;
FSvar * parptr[];
{
	fn_stack fn_stk;
	FSchar *__who_me=fgs_fn_start(&fn_stk,fgs_file_name,"test",3,parno,parptr);

	FSvar *par0[8], *par1[2];
	if(fgsDonotenter) init_ref_data();
	_fgs_check_event(3,1,100);
#line 3 "test.prg"
	par0[0] = cre_tmpvar();
	par0[1] = cre_tmpvar();
	par0[2] = cre_tmpvar();
	par0[3] = cre_tmpvar();
	par0[4] = mk_var_cp(fgsGetVar(_bbvar_nil));
	par0[5] = mk_var_cp(fgsGetVar(_bbvar_nil));
	par0[6] = set_cv("Hello world!");
	par0[7] = _bb_qout6(7, &par0[0]);
	_fgs_check_event(4,1,100);
#line 4 "test.prg"
	par0[0] = cre_tmpvar();
	par0[1] = cre_tmpvar();
 ...
 if(0) {
	...
	par1[0] = mv_names[_bbvar_nil].v;
 }
	par0[6] = mk_var_cp(par1[0]);
	par0[7] = _bb___wait(7, &par0[0]);
	_fgs_check_event(5,1,100);
#line 5 "test.prg"
	par0[0] = _bb___quit(0, (FSvar**)0);
	_fgs_check_event(6,1,100);
#line 6 "test.prg"
_adios:
